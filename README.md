...
> **环境要求**：Python >= 3.8，CUDA >= 11.7，显存 >= 24GB

### 1. 环境准备

确保您的机器已安装 Python 3.8+、CUDA 环境及相关显卡驱动，执行以下命令安装核心框架：
...
2. **必须修改** `data/dataset_info.json`，追加以下注册信息，否则框架无法识别数据：
...
训练完成后，使用以下脚本加载微调后的 Adapter 进行效果测试：
...
1. **显存优化与稳定性控制**：在单卡环境下，通过设置 `per_device_train_batch_size=2` 并配合梯度累积（Gradient Accumulation），有效平衡内存吞吐量与梯度下降稳定性，规避了 OOM 报错。

2. **认知注入与收敛控制**：针对极小样本数据集，设置学习率 `LR=2e-4`，训练 40 个 Epoch，Train Loss 从初始 3.35 收敛至 0.45，模型在测试集上稳定输出预设身份回复。

3. **网络链路优化**：引入 `USE_MODELSCOPE_HUB=1` 环境变量，解决了特定网络环境下从 HuggingFace 拉取权重缓慢及超时的问题。
